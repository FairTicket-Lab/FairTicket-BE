...
public interface ReservationRepository extends ReactiveCrudRepository<Reservation, Long> {
    ...
    @Query("SELECT COALESCE(SUM(quantity), 0) FROM reservations WHERE schedule_id = :scheduleId AND grade = :grade AND track_type = 'LOTTERY' AND status IN ('PENDING', 'PAID_PENDING_SEAT')")
    ...
    @Query("SELECT COALESCE(SUM(quantity), 0) FROM reservations WHERE schedule_id = :scheduleId AND user_id = :userId AND track_type = 'LOTTERY' AND status IN ('PENDING', 'PAID_PENDING_SEAT')")
    ...
    @Query("SELECT COALESCE(SUM(quantity), 0) FROM reservations WHERE schedule_id = :scheduleId AND user_id = :userId AND track_type = 'LIVE' AND status IN ('PENDING', 'PAID_PENDING_SEAT', 'ASSIGNED')")
    ...
    @Query("SELECT grade, COALESCE(SUM(quantity), 0) as total FROM reservations WHERE schedule_id = :scheduleId AND track_type = 'LOTTERY' AND status IN ('PENDING', 'PAID_PENDING_SEAT') GROUP BY grade")
